...
public class UploadServiceImpl implements UploadService {
    ...
    @Override
    public void uploadCsvFiles(MultipartFile srvyBscFile, MultipartFile srvyCbookFile, MultipartFile srvyCtgryFile, MultipartFile srvyRspnsFile) throws IOException {
        validateFile(srvyBscFile, SRVY_BSC_DIR);
        validateFile(srvyCbookFile, SRVY_CBOOK_DIR);
        validateFile(srvyCtgryFile, SRVY_CTGRY_DIR);
        validateFile(srvyRspnsFile, SRVY_RSPNS_DIR);

        storeFile(srvyBscFile, SRVY_BSC_DIR);
        storeFile(srvyCbookFile, SRVY_CBOOK_DIR);
        storeFile(srvyCtgryFile, SRVY_CTGRY_DIR);
        storeFile(srvyRspnsFile, SRVY_RSPNS_DIR);
    }

    private void validateFile(MultipartFile file, Path dirName) throws IOException {
    ...
    private void storeFile(MultipartFile file, Path dirName) throws IOException {
        String originalFilename = StringUtils.cleanPath(
                Objects.requireNonNull(file.getOriginalFilename())
        );

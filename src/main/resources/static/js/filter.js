    ...
    config.selects.forEach(selectConfig => {
        ...
        if (element && data) {
            ...
            selectOrder.push({
                ...
                data: data,
                excludeFromParams: selectConfig.excludeFromParams || false
            });
        }
    });
    ...
    if (btnSearch) {
        btnSearch.addEventListener("click", () => {
            ...
            selectOrder.forEach(selectInfo => {
                if (selectInfo.excludeFromParams) {
                    return; // Skip if configured to be excluded
                }
                const select = selects[selectInfo.key];
                if (select && select.value) {
                    const selectedOption = select.options[select.selectedIndex];
                    // Use dataset.value if it exists, otherwise use the option's value
                    const valueToUse = selectedOption.dataset.value !== undefined ? selectedOption.dataset.value : select.value;
                    selectedValues[selectInfo.key] = valueToUse;
                }
            });
